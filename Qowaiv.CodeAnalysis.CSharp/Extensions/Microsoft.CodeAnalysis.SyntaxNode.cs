...
public static class SyntaxNodeExtensions
{
    ...
    extension(SyntaxNode node)
    {
        ...
        [Pure]
        public PropertyDeclaration PropertyDeclaration(SemanticModel model)
            => new(node.Cast<PropertyDeclarationSyntax>(), model);

        [Pure]
        public TypeDeclaration TypeDeclaration(SemanticModel model)
        ...
        [Pure]
        public TypeDeclaration? TryTypeDeclaration(SemanticModel model) => node switch
        {
        ...
        [Pure]
        public MemberDeclaration MemberDeclaration(SemanticModel model)
        ...
        [Pure]
        public MemberDeclaration? TryMemberDeclaration(SemanticModel model) => node switch
        {
        ...
        [Pure]
        public TypeNode TypeNode(SemanticModel model)
            => TryTypeNode(node, model)
            ?? throw new InvalidOperationException($"Unexpected {node.GetType().Name}, expected a type declaration.");

        [Pure]
        public TypeNode? TryTypeNode(SemanticModel model) => node switch
        {
